...
void TextureManager::DrawFrame(std::string id, int x, int y, int width, int height, int row, int frame, SDL_RendererFlip flip,SDL_Point* center)
{
	...
	height * (row-1) lay toa do y cua hang thu row - 1

	*/
	SDL_Rect srcRect = { width * frame, height * (row - 1) , width, height };
	SDL_Rect dstRect = { x,y,width,height };
	/*
	SDL_RenderCopyEx(): sao chep 1 phan cua texture den vt hien tai
	SDL_Rect: 1 cau truc dinh nghia 1 hinh chu nhat voi goc o goc tren ben trai->chua toa do x,y,width,height
